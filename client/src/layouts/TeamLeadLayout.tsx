...
const ALL_TEAM_LEAD_HREFS: string[] = [];
TEAM_LEAD_NAV.forEach(section => {
  section.items.forEach((item: any) => {
    if (item.href) ALL_TEAM_LEAD_HREFS.push(item.href);
    if (item.subItems) {
      item.subItems.forEach((sub: any) => {
        if (sub.href) ALL_TEAM_LEAD_HREFS.push(sub.href);
      });
    }
  });
});
          ...
          <div key={section.label}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item: any) => {
                      ...
                      {isOpen && sidebarOpen && (
                        <div className="ml-3 mt-1 space-y-1 border-l border-border pl-3">
                          {item.subItems.map((sub: any) => {
                            ...
                            const active = isItemActive(sub.href, pathname, ALL_TEAM_LEAD_HREFS);
                            return (
                              <NavLink
                                key={sub.href}
                                to={sub.href}
                ...
                const active = isItemActive(item.href, pathname, ALL_TEAM_LEAD_HREFS);
                return (
                  <NavLink
                    key={item.href}
                    to={item.href}
