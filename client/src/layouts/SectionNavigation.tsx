...
export function SectionRail({ id, groups, open, onNavigate }: { id: string; groups: SectionGroup[]; open: boolean; onNavigate?: () => void }) {
  ...
  return <nav aria-label="Main navigation" className="no-scrollbar flex-1 space-y-1 overflow-y-auto px-3 py-3 md:px-2">
    {groups.map(group => {
      ...
      return <button key={group.label} type="button" title={group.label} aria-label={group.label}
        ...
        className={cn('flex min-h-11 w-full flex-row items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors md:min-h-16 md:flex-col md:justify-center md:gap-1 md:rounded-xl md:px-1 md:text-center',
          active ? 'bg-primary/10 text-primary dark:bg-primary/20' : 'text-muted-foreground hover:bg-muted hover:text-foreground')}>
        {Icon && <Icon className="size-4 shrink-0 md:size-5" />}
        {open && <span className="w-full truncate text-sm font-medium leading-tight md:text-[11px]">{group.label.replace(/^(MY |EMPLOYEE )/, '')}</span>}
      </button>;
    })}
  </nav>;
}
