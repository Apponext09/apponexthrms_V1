...
export function WorkflowSettingsDrawer({ open, onClose, editingWorkflow, onSaved }: Props) {
  const [activeTab, setActiveTab] = useState<DrawerTab>('workflow');
  ...
  const handleAddStepClick = (type: 'reporting_officer' | 'employee' | 'department' | 'role') => {
    if (type === 'reporting_officer') {
      ...
      toast.success('Added Reporting Officer step');
    } else {
