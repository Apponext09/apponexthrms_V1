      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <Layers className="w-5 h-5 text-indigo-400" /> Available Subscription Plans
          </h2>
          <span className="text-xs text-slate-400">{plans.length} Configured Tiers</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <Card
              ...
              className="bg-slate-900 border border-slate-800 text-white shadow-xl flex flex-col justify-between hover:border-slate-700 transition"
            >
              <div>
                <CardHeader className="pb-3">
                  ...
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] text-slate-400 bg-slate-950/60 p-2 rounded-lg border border-slate-800">
                  ...
                  <CardDescription className="text-slate-400 text-xs mt-3 leading-relaxed">
                    {plan.description || 'No description provided.'}
                  </CardDescription>
                </CardHeader>

                <CardContent className="py-3 border-t border-slate-800/80 space-y-2">
                  <p className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Included Admin Modules ({plan.modules?.length || 0})
                  </p>
                  <div className="space-y-1.5 pt-1">
                    {plan.modules && plan.modules.length > 0 ? (
                      plan.modules.map((mod) => (
                        <div key={mod} className="text-xs text-slate-300 flex items-center gap-2">
                          <CheckCircle className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                          <span>{mod}</span>
                        </div>
                      ))
                    ) : (
                      <p className="text-xs text-slate-500 italic">No specific modules selected.</p>
                    )}
                  </div>
                </CardContent>
              </div>
              ...
              <CardFooter className="pt-4 border-t border-slate-800">
                <Button
                  ...
                  variant="outline"
                  className="w-full border-slate-700 text-slate-200 hover:bg-slate-800 hover:text-white font-semibold text-xs gap-1.5"
                >
                  <Edit className="w-3.5 h-3.5 text-indigo-400" /> Edit Plan Tier
                </Button>
              </CardFooter>
            </Card>
          ))}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-300 font-semibold">Subscription Plan Name *</Label>
                <Input
                  required
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-300 font-semibold">Pricing (₹ or Custom) *</Label>
                <Input
                  required
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-300 font-semibold">Status *</Label>
                <select
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800">
              <div className="flex items-center justify-between">
                <Label className="text-xs text-slate-200 font-bold flex items-center gap-1.5">
                  <Layers className="w-4 h-4 text-indigo-400" /> Assign Admin Modules (Checkboxes)
                </Label>
