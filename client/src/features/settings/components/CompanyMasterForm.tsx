...
const INITIAL_COMPANIES: CompanyRecordItem[] = [
  {
    id: 'comp-1',
    code: 'HQ-MAIN',
    name: 'Trial Company',
...
export function CompanyMasterForm({
  ...
  const [companies, setCompanies] = useState<CompanyRecordItem[]>(companiesList || INITIAL_COMPANIES);
  ...
  const [selectedId, setSelectedId] = useState<string>(companiesList?.[0]?.id || 'comp-1');
      ...
      try {
        ...
        if (res.data?.success && Array.isArray(res.data.data) && res.data.data.length > 0) {
          const mapped: CompanyRecordItem[] = res.data.data.map((c: any) => ({
            id: String(c.company_id || c.id || c.uuid),
            code: c.code,
            name: c.name,
            employerName: c.employer_name || '',
            classOfEstablishment: c.class_of_establishment || '',
            ...
            email: c.email || '',
            logo: c.logo || '',
            companyStamp: c.company_stamp || '',
            signature: c.signature || '',
            isActiveToggle: c.is_active_toggle === 1 || c.is_active_toggle === true,
            activeUsersToggle: c.active_users_toggle === 1 || c.active_users_toggle === true,
            loginPageLogoToggle: c.login_page_logo_toggle === 1 || c.login_page_logo_toggle === true,
        ...
        console.warn('Fallback to local state (DB endpoint unreachable):', err);
      }
    };
    fetchDbCompanies();
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    let savedCompany: CompanyRecordItem;

    try {
      const isUpdating = !isNew && selectedId && !selectedId.startsWith('comp-');
      let res;
      if (isUpdating) {
      ...
      if (res.data?.success && res.data.data) {
        ...
        savedCompany = {
          id: String(c.companyId || c.company_id || c.id || `comp-${Date.now()}`),
          code: c.code,
          name: c.name,
          employerName: c.employerName || c.employer_name || formEmployerName,
          classOfEstablishment: c.classOfEstablishment || c.class_of_establishment || formClassOfEstablishment,
          addressLine1: c.addressLine1 || c.addressLine_1 || c.address_line_1 || formAddress1,
          addressLine2: c.addressLine2 || c.addressLine_2 || c.address_line_2 || formAddress2,
          country: c.country || formCountry,
          zipCode: c.zipCode || c.zip_code || formZipCode,
          state: c.state || formState,
          city: c.city || formCity,
          panTin: c.panTin || c.pan_tin || formPanTin,
          contactNumber: c.contactNumber || c.contact_number || formContactNumber,
          email: c.email || formEmail,
          logo: c.logo || formLogo,
          companyStamp: c.companyStamp || c.company_stamp || formCompanyStamp,
          signature: c.signature || formSignature,
          isActiveToggle: c.isActiveToggle !== undefined ? Boolean(c.isActiveToggle) : formIsActiveToggle,
          activeUsersToggle: c.activeUsersToggle !== undefined ? Boolean(c.activeUsersToggle) : formActiveUsersToggle,
          loginPageLogoToggle: c.loginPageLogoToggle !== undefined ? Boolean(c.loginPageLogoToggle) : formLoginPageLogoToggle,
          status: c.status || formStatus,
        };
      } else {
        savedCompany = { id: isNew ? `comp-${Date.now()}` : selectedId, ...payload };
      }
    } catch (err) {
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-foreground">
            {isNew ? 'Add New Company' : 'Add Company Information'}
          </h2>
        </div>
