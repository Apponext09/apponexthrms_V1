...
export function HRConfigurationPage() {
  ...
  const handleSaveCurrentTab = async () => {
    ...
    try {
      ...
      toast.success(`${activeTabMeta.label} settings saved successfully!`);
    } catch (err) {
          ...
          <Card className="rounded-2xl border shadow-2xs overflow-hidden">
            ...
            <CardContent className="p-6">
              ...
              {activeTab === 'notification-setting' && (
                <div className="space-y-6">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-rose-600">Notification Delivery Channels</h4>
                    ...
                    {formTypes.map((f) => (
                      <div key={f.id} className="flex items-center justify-between px-4 py-3 bg-card">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => handleDeleteFormType(f.id)} className="text-red-500 hover:text-red-700 p-1">
                  ...
                  <div className="border rounded-xl overflow-hidden divide-y text-xs">
                    {allowedIps.map((item) => (
                      <div key={item.id} className="flex items-center justify-between px-4 py-3 bg-card">
                        ...
                        <button onClick={() => handleDeleteIp(item.id)} className="text-red-500 hover:text-red-700 p-1">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
