...
export function MyLifecyclePage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-3 sm:p-5 lg:p-6">
    <section className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${stage.accent} px-5 py-6 text-white shadow-lg sm:px-7`}><div className="absolute -right-12 -top-20 h-52 w-52 rounded-full bg-white/10" /><div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/70">Employment journey</p><h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">My Lifecycle</h1><p className="mt-2 text-sm text-white/80">Your verified employment milestones and current assignment.</p></div><div className="flex items-center gap-2"><Badge variant="outline" className="border-white/30 bg-white/15 px-3 py-1 text-white">{stage.label}</Badge><Button size="sm" variant="secondary" onClick={fetchLifecycle} className="gap-1.5"><RefreshCw className="h-3.5 w-3.5" /> Refresh</Button></div></div></section>
