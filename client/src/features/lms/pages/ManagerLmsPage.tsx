...
export function ManagerLmsPage() {
  ...
  const { employees: teamMembers = [] } = useManager();
  const teamMemberIds = teamMembers.map((m: any) => m.id);

  const { data: courses = [] } = useLmsCourses({ status: 'published' });
  const { data: teamEnrollments = [], isLoading: isTeamLoading } = useLmsEnrollments({
    search,
  });
  const { data: myEnrollments = [] } = useMyLmsEnrollments(managerId);

  const enrollMutation = useEnrollCourse();

  // Filter enrollments to team members
  const memberEnrollments = teamEnrollments.filter((e) =>
  ...
  return (
    <div className="p-6 space-y-6 bg-background">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card border border-border/80 rounded-xl p-5 shadow-2xs">
        <div>
          <h2 className="text-xl font-black text-foreground tracking-tight flex items-center gap-2">
            <Users className="w-5 h-5 text-primary" /> Department Team Learning & Skill Matrix
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Monitor team training progress, suggest relevant upskilling courses, and track compliance.
          </p>
        </div>
        ...
        >
          <Plus className="w-4 h-4" /> Suggest / Assign Course
        </Button>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-border pb-2">
        ...
        >
          <Users className="w-3.5 h-3.5" /> Team Member Progress ({memberEnrollments.length})
        </Button>
        <Button
          size="sm"
        ...
        <Button
          ...
          className="text-xs font-bold gap-1.5"
        >
          <PlayCircle className="w-3.5 h-3.5" /> My Personal Learning ({myEnrollments.length})
        </Button>
      </div>
            ...
            <CardContent className="p-0">
              {isTeamLoading ? (
                ...
                <div className="py-16 text-center text-xs text-muted-foreground">
                  ...
                  No course enrollments for your department members yet. Click "Suggest / Assign Course" to assign training.
                </div>
              ) : (
        ...
        <Card className="max-w-xl border border-border/80 rounded-xl shadow-2xs bg-card p-6">
          <CardHeader className="p-0 pb-4 border-b border-border/60">
            ...
            <CardDescription className="text-xs">
              Directly enroll a subordinate into technical or compliance training
            </CardDescription>
          </CardHeader>
              ...
              >
                ...
                {teamMembers.map((emp: any) => (
                  <option key={emp.id} value={emp.id}>
                    ...
                    {emp.employeeCode || emp.employee_code || emp.designation})
                  </option>
                ))}
              </select>
            </div>
      ...
      {/* ── Tab 3: Manager's Personal Learning ────────────────── */}
      {activeTab === 'my' && (
