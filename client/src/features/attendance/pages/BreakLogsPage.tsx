...
const getRowEmpName = (r: BreakLogRow) =>
  r.employeeName || r.employee_name || "Employee";
const getRowEmpCode = (r: BreakLogRow) =>
  r.employeeCode || r.employee_code || "";
const getRowBreakType = (r: BreakLogRow) =>
  r.breakTypeName || r.break_type_name || "General Break";
const getRowStartTime = (r: BreakLogRow) =>
  r.breakStartTime || r.break_start_time || null;
const getRowEndTime = (r: BreakLogRow) =>
  r.breakEndTime || r.break_end_time || null;
const getRowDuration = (r: BreakLogRow) =>
  r.breakDurationMinutes ?? r.break_duration_minutes ?? null;
...
export const BreakLogsPage: React.FC = () => {
  ...
  const [employees, setEmployees] = useState<
    { id: number; name: string; code?: string; reportingManagerId?: number }[]
  >([]);
  ...
  const loadMasterFilters = useCallback(async () => {
    try {
      const [deptRes, empRes] = await Promise.all([
        apiClient
          .get("/settings/departments?pageSize=100")
          .catch(() => ({ data: { data: [] } })),
        apiClient
          .get("/employees?pageSize=200&is_ceo=0")
          .catch(() => ({ data: { data: [] } })),
      ]);

      const deptList = (deptRes.data?.data || deptRes.data?.items || [])
        .map((d: any) => ({
      ...
      const empList = (empRes.data?.data || empRes.data?.items || [])
        .filter((e: any) => {
          const isCeo = Boolean(
            ...
            e.isCeo === 1 ||
            e.is_ceo === 1 ||
            e.accessRole === "organization_admin" ||
            e.access_role === "organization_admin" ||
            e.role === "organization_admin",
          );
        ...
        .map((e: any) => ({
          ...
          name:
            e.first_name || e.firstName || e.lastName
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      if (request === latestRequest.current) setRows(res.data?.data || []);
    } catch {
  ...
  return (
    <div className="-m-4 space-y-5 bg-[#F2F7FD] p-4 pb-12 font-['Plus_Jakarta_Sans',ui-sans-serif,system-ui,sans-serif] sm:-m-6 sm:p-6 dark:bg-background">
      ...
      <AttendancePageHeader
        icon={Coffee}
        title="Break Logs"
        description="Review employee breaks, filter records, and export the visible results."
        actions={
          <>
            <Button
              size="sm"
            ...
            >
              <RefreshCw
                className={`w-3.5 h-3.5 text-primary ${loading ? "animate-spin" : ""}`}
              />
              Refresh
            </Button>
            <Button
      ...
      {loadError && (
        <div
          role="alert"
          className="flex items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"
        >
          <span>Could not load break logs. Please try again.</span>
      ...
      <div className="space-y-4 rounded-2xl border border-blue-100 bg-white p-4 shadow-sm dark:border-border dark:bg-card">
      ...
      {startDate > endDate && <p role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">The start date must be on or before the end date.</p>}

      {/* Data table */}
      <div className="overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-sm dark:border-border dark:bg-card">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border/60 bg-muted/30">
                ...
                <th className="text-right px-4 py-2.5 font-bold text-muted-foreground uppercase tracking-wider">
                  Duration
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12">
                    ...
                    <Button type="button" size="sm" variant="outline" onClick={handleResetFilters} className="mt-3">Clear filters</Button>
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-black ${durationBadgeColor(duration)}`}
                        >
