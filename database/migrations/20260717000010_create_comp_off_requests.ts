...
export async function up(knex: Knex): Promise<void> {
  const exists = await knex.schema.hasTable('comp_off_requests');
  if (exists) return;

  await knex.schema.createTable('comp_off_requests', (table) => {
    table.bigIncrements('id').primary();
    table.uuid('uuid').notNullable().unique();
    table.bigInteger('organization_id').unsigned().notNullable();
    table.bigInteger('employee_id').unsigned().notNullable();
    table.bigInteger('comp_off_id').unsigned().notNullable();
    ...
    table.foreign('comp_off_id').references('comp_off_balances.id');
