...
export async function up(knex: Knex): Promise<void> {
  const exists = await knex.schema.hasTable('payroll_earnings');
  if (exists) return;

  await knex.schema.createTable('payroll_earnings', (table) => {
    ...
    table.string('formula_used', 255).nullable();
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.dropTableIfExists('payroll_earnings');
}
