...
async function updateEpsWagesAndAddEsiWages() {
  ...
  console.log('=== UPDATING EPS WAGES & ADDING ESI WAGES ===\n');

  // 1. Update EPS wages formula to [BASIC_EARNED]
  await conn.query(
    `UPDATE payroll_components 
     SET formula = '[BASIC_EARNED]',
         component_type = 'Derived',
         boundary_type = 'Fixed',
         based_on_attendance = 0,
         is_active = 1
     WHERE name LIKE '%EPS wages%'`
  );
  console.log('✅ Updated "EPS wages" -> Derived: [BASIC_EARNED]');

  // 2. Group: "ESI Wages"
  let [esiwGroup] = await conn.query(
    'SELECT id FROM payroll_component_groups WHERE name = ? AND organization_id = ? AND category = \"Deduction\" AND deleted_at IS NULL',
    ['ESI Wages', orgId]
  );
  let esiwGroupId;
  if (esiwGroup.length === 0) {
    const [res] = await conn.query(
      `INSERT INTO payroll_component_groups 
       ...
       VALUES (?, ?, 'ESI Wages', 'Deduction', 'Round', 'Max', 0, 0, 1, 'Employer', 1, 'Choose', 10, 1, 0, 0, 0)`,
      [uuidv4(), orgId]
    );
    esiwGroupId = res.insertId;
    console.log(`✅ Created Deduction Group: "ESI Wages" (ID: ${esiwGroupId})`);
  } else {
    esiwGroupId = esiwGroup[0].id;
  }

  let [esiwComp] = await conn.query(
    'SELECT id FROM payroll_components WHERE name = ? AND group_id = ? AND deleted_at IS NULL',
    ['ESI Wages', esiwGroupId]
  );
  if (esiwComp.length === 0) {
    const [res] = await conn.query(
      `INSERT INTO payroll_components 
       ...
       VALUES (?, ?, ?, 'ESI Wages', 'Derived', 0.00, '[GROSS_EARNED]', 0, 0, 'Fixed', 1)`,
      [uuidv4(), orgId, esiwGroupId]
    );
    console.log(`  └─ Created Component: "ESI Wages" [Derived: [GROSS_EARNED]] (ID: ${res.insertId})`);
  }
...
updateEpsWagesAndAddEsiWages().catch(err => {
