...
async function seedCleanGroupsAndComponents() {
  console.log('--- Setting up Clean Component Groups & Components ---');

  // 1. Create or Find Component Groups
  ...
  console.log('Groups configured successfully.');
  ...
  for (const c of allComponents) {
    ...
    console.log(`Configured Component ${c.id}: ${c.name} (${c.component_type})`);
  }

  // 3. Update active slab 2 ("Monthly") to include all components
  await db('payroll_slabs')
    .where('id', 2)
    .update({
      selected_component_ids: JSON.stringify(allIds),
      updated_at: new Date()
    });
  console.log(`Updated Slab 2 (Monthly) with ${allIds.length} components.`);

  console.log('--- All Component Groups & Components Seeded Successfully ---');
  process.exit(0);
}

seedCleanGroupsAndComponents().catch(e => {
  console.error(e);
  process.exit(1);
});
