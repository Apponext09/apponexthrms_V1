...
export async function up(knex: Knex): Promise<void> {
  const exists = await knex.schema.hasTable('tax_declarations');
  if (exists) return;
  ...
  await knex.schema.createTable('tax_declarations', (table) => {
    ...
    table.unique(['organization_id', 'employee_id', 'financial_year'], { indexName: 'tax_decl_org_emp_year_unique' });
