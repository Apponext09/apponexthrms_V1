...
} from "../repositories/AttendanceRecordRepository";
...
const computeShiftEntryStatus = (
  ...
  const graceMins = Math.max(
    0,
    Number(shift.grace_period_minutes ?? shift.gracePeriodMinutes ?? 0),
  );
  const durationHours = Number(
    shift.duration_hours ?? shift.durationHours ?? 8.5,
  );
  ...
  const rawHalfDayTime =
    (shift as any).half_day_start_time ||
    (shift as any).halfDayStartTime ||
    rosterRules?.halfDayStartTime ||
    rosterRules?.half_day_start_time;
  let halfDayDeadline: Date;
  if (rawHalfDayTime) {
    const parsedHalfDay = parseShiftTime(rawHalfDayTime, todayStr);
    halfDayDeadline =
      parsedHalfDay || new Date(shiftStart.getTime() + 2 * 60 * 60 * 1000);
  } else {
  ...
  const lateMinutes =
    nowMs > shiftMs ? Math.floor((nowMs - shiftMs) / 60000) : 0;
  ...
  console.log(
    `[ShiftEntryCalc] ShiftStart: ${fmt(shiftStart)} | GraceDeadline: ${res.graceDeadlineLabel} | HalfDayDeadline: ${res.halfDayDeadlineLabel} | Now: ${fmt(checkInNow)} → Result: ${res.entryStatus} (${res.status}, isLate: ${res.isLate})`,
  );
  return res;
};
...
export class AttendanceService {
  ...
  private async assertLocationInputWhenRequired(
    ctx: TenantContext,
    employeeId: number,
    latitude?: number,
    longitude?: number,
  ): Promise<void> {
    const mapping = await getKnex()("employee_attendance_locations")
      .where("organization_id", ctx.organizationId)
      .where("employee_id", employeeId)
      .first("id")
      .catch((error: any) => {
        logger.error(
          "[AttendanceService] Could not resolve employee location requirement",
          error,
        );
        throw new ValidationError(
          "Unable to verify attendance location policy. Please try again.",
        );
      });
    if (mapping && (latitude == null || longitude == null)) {
      throw new ValidationError(
        "Location permission is required to mark attendance for this employee.",
      );
    }
    if ((latitude == null) !== (longitude == null)) {
      throw new ValidationError(
        "Both latitude and longitude are required for location validation.",
      );
    }
  }
  ...
  private async resolveHolidayStatus(
    ...
    try {
      ...
      const calResult = await holidayCalSvc.getCalendarForEmployee(
        null,
        ctx,
        employeeId,
      );
      if (!calResult) {
        ...
        return {
          ...
          isWeekOff: false,
          isPunchAllowed: true,
          punchAllowedReason: "No holiday calendar assigned",
        };
      }

      // 2. Check if today is a holiday or week-off
      const offDayResult = await holidayCalSvc.isHolidayOrWeekOff(
        null,
        ctx,
        calResult.calendarId,
        date,
      );
      ...
      if (isHoliday && offDayResult.isOptional) {
        return {
          ...
          isPunchAllowed: true,
          punchAllowedReason:
            "Optional/Restricted Holiday does not block check-in",
        };
      }
      ...
      const orgSetting = await db("organization_leave_settings")
        ...
        .catch(() => null);
      const orgAllowsHolidayPunch =
        !!orgSetting?.allow_holiday_punch_by_default;
      ...
      const statusVal =
        overtimeReq?.approvalStatus || overtimeReq?.approval_status;
      ...
      console.warn(
        "[AttendanceService] resolveHolidayStatus error (non-fatal):",
        e,
      );
      return {
        isHoliday: false,
  ...
  async checkIn(
    ...
    if (!assignedShiftId) {
      try {
        const assignedShift = await this.shiftService.getEmployeeShift(
          ctx,
          input.employeeId,
          today,
        );
        ...
        console.warn(
          "[AttendanceService] Failed to fetch assigned shift for employee check-in:",
          shiftErr,
        );
      }
    } else {
      ...
      try {
        resolvedShift = await this.shiftService.getShiftById(
          ctx,
          assignedShiftId,
        );
      } catch {}
    ...
    // A punch after midnight may still belong to yesterday's overnight shift.
    const currentMoment = new Date();
    const yesterdayDate = new Date(currentMoment);
    yesterdayDate.setDate(yesterdayDate.getDate() - 1);
    const yesterday = getLocalYYYYMMDD(yesterdayDate);
    if (!input.shiftId) {
      const previousShift = await this.shiftService
        .getEmployeeShift(ctx, input.employeeId, yesterday)
        .catch(() => null);
      if (previousShift) {
        const previousWindow = resolveShiftWindow(previousShift, yesterday);
        const previousRules = getShiftRules(previousShift);
        const cutoff = previousRules.shiftCutOffTime
          ? parseShiftTime(previousRules.shiftCutOffTime, today)
          : previousWindow.end;
        if (
          previousWindow.start &&
          cutoff &&
          currentMoment >= previousWindow.start &&
          currentMoment <= cutoff
        ) {
          const existingPrevious = await this.recordRepo.getByEmployeeAndDate(
            ctx,
            input.employeeId,
            yesterday,
          );
          if (
            existingPrevious &&
            !(existingPrevious.checkOutTime ?? existingPrevious.check_out_time)
          ) {
            throw new ValidationError(
              "An attendance session for the previous night shift is still open. Please check out first.",
            );
          }
          today = yesterday;
          resolvedShift = previousShift;
          assignedShiftId =
            previousShift.shift_id ||
    ...
    await this.assertLocationInputWhenRequired(
      ctx,
      input.employeeId,
      input.latitude,
      input.longitude,
    ...
    if (input.latitude != null && input.longitude != null) {
      try {
        const geoValidation =
          await this.geofenceService.validateCheckInLocation(
            ctx,
            input.employeeId,
            input.latitude,
            input.longitude,
        ...
        throw new ValidationError(
          "Unable to verify attendance location. Please enable location access and try again.",
        );
      }
    }
    ...
    if (matchedLocationId) {
      try {
        const locExists = await this.geofenceRepo.getById(
          ctx,
          matchedLocationId,
        );
    ...
    const holidayStatus = await this.resolveHolidayStatus(
      ctx,
      input.employeeId,
      today,
    );
    ...
    if (resolvedShift && !isRestrictedDay) {
      const workingDate = new Date(`${today}T12:00:00`);
      if (!isShiftWorkingDay(workingDate, resolvedShift)) {
        throw new ValidationError(
          "The assigned shift marks this date as a roster rest day. Contact HR if you are scheduled to work.",
        );
      }
    }
    ...
    const holidayNote =
      isRestrictedDay && holidayStatus.isPunchAllowed
    ...
    const entryResult = resolvedShift
      ? evaluateShiftEntry(resolvedShift, today, checkInDateObj)
      : computeShiftEntryStatus({}, checkInDateObj, today);

    const attendanceStatus = entryResult.status; // 'present' | 'half_day'
    const isLateFlag = entryResult.isLate;
    const entryNotes =
      entryResult.entryStatus === "late"
    ...
    const finalNotes = holidayNote
      ? entryNotes
        ? `${holidayNote} | ${entryNotes}`
        : holidayNote
      : entryNotes;

    let record = await this.recordRepo.getByEmployeeAndDate(
      ctx,
      input.employeeId,
      today,
    );
    ...
    if (isLateFlag) {
      try {
        ...
        if (lateUpdationRules.length > 0) {
          ...
          if (employee) {
            ...
            for (const rule of lateUpdationRules) {
              ...
              if (
                ruleDepartments.length > 0 &&
                !ruleDepartments.includes(
                  Number(employee.current_department_id),
                )
              )
                continue;
              ...
              if (ruleGrades.length > 0 && !ruleGrades.includes(employee.grade))
                continue;
              ...
              if (
                ...
                !ruleStatuses.includes(employee.status)
              )
                continue;
              ...
              if (ruleShifts.length > 0) {
                ...
                if (
                  !shiftAssignment ||
                  !ruleShifts.includes(Number(shiftAssignment.shift_id))
                )
                  continue;
              }
              ...
              if (rule.auto_apply_leave) {
                try {
                  const hasLedger = await db.schema.hasTable(
                    ...
                    const lwpType = await db("leave_types")
                      ...
                      .where(function (this: any) {
                        this.whereRaw("LOWER(leave_name) = 'lwp'")
                          ...
                          .orWhereRaw(
                            "LOWER(leave_name) = 'leave without pay'",
                          );
                      })
                      .first();
                  ...
                  console.warn(
                    "[AttendanceService] Late auto-deduction leave ledger error:",
                    ledgerErr,
                  );
                }
              }
        ...
        console.warn(
          "[AttendanceService] Late auto-deduction assignment error:",
          lateErr,
        );
      }
    }
    ...
    if (isLateFlag && entryResult.lateMinutes > 0) {
      try {
        ...
        logger.warn(
          "[AttendanceService] Late-mark notification failed (non-fatal)",
          e,
        );
      }
    }
  ...
  async checkOut(
    ...
    if (!record) {
      ...
      const previous = getLocalYYYYMMDD(previousDate);
      const previousRecord = await this.recordRepo.getByEmployeeAndDate(
        ctx,
        input.employeeId,
        previous,
      );
      if (
        previousRecord &&
        !(previousRecord.checkOutTime ?? previousRecord.check_out_time)
      ) {
        const priorShiftId = previousRecord.shiftId ?? previousRecord.shift_id;
        const priorShift = priorShiftId
          ? await this.shiftService.getShiftById(ctx, priorShiftId)
          : null;
        const window = priorShift
          ? resolveShiftWindow(priorShift, previous)
          : null;
        const cutoff =
          priorShift && getShiftRules(priorShift).shiftCutOffTime
            ? parseShiftTime(
                getShiftRules(priorShift).shiftCutOffTime,
                getLocalYYYYMMDD(),
              )
            : window?.end;
        if (
    ...
    const openBreak = openBreaks.find(
      (b) => b.status === "active" || b.status === "paused",
    );
    if (openBreak) {
      const rawStartTime =
        openBreak.break_start_time ||
        (openBreak as any).breakStartTime ||
        openBreak.created_at;
      ...
      if (rawStartTime instanceof Date) {
        ...
        const isoStart = rawStartTime.includes("T")
          ? rawStartTime
      ...
      if (
        !isNaN(breakStartTime) &&
        !isNaN(breakEndTime) &&
        breakEndTime > breakStartTime
      ) {
        breakDurationMinutes = Math.max(
          1,
          Math.floor((breakEndTime - breakStartTime) / (1000 * 60)),
        );
      }
    ...
    const totalBreakMinutes = await this.breakRepo.getTotalBreakDuration(
      ctx,
      record.id,
    );
    // workDurationMinutes = gross duration minus any actual breaks taken
    const recordShiftId = record.shiftId ?? record.shift_id;
    const checkoutShift = recordShiftId
      ? await this.shiftService.getShiftById(ctx, recordShiftId)
      : await this.shiftService
          .getEmployeeShift(ctx, input.employeeId, today)
          .catch(() => null);
    const isExcludedShiftDay = checkoutShift
      ? !isShiftWorkingDay(new Date(`${today}T12:00:00`), checkoutShift)
      : false;
    const checkoutEvaluation = checkoutShift
      ? evaluateShiftCheckout(
          checkoutShift,
          today,
          durationMinutes,
    ...
    const workDurationMinutes =
      checkoutEvaluation?.workMinutes ??
      Math.max(0, durationMinutes - totalBreakMinutes);
    ...
    try {
      ...
      if (otRule) {
        ...
        let shiftStart = shiftRow?.start_time
          ? parseShiftTime(String(shiftRow.start_time), today)
          : null;
        let shiftEnd = shiftRow?.end_time
          ? parseShiftTime(String(shiftRow.end_time), today)
          : null;
        ...
        if (shiftRow?.shift_type === "roster" || shiftRow?.roster_pattern) {
          const isWorking = this.shiftService.isWorkingDay(
            dateObj,
            shiftRow.roster_pattern,
          );
          ...
          try {
            ...
            if (weekOffRule) {
              const isAlt = Boolean(
                weekOffRule.is_alternate || weekOffRule.isAlternate,
              );
              if (isAlt) {
                const dayOfMonth = dateObj.getDate();
                const nthWeekday = Math.floor((dayOfMonth - 1) / 7) + 1;
                const altWeeksStr = String(
                  weekOffRule.alternate_weeks ||
                    weekOffRule.alternateWeeks ||
                    "",
                );
        ...
        const dayType: "normal" | "holiday" | "weekend" = isHoliday
          ? "holiday"
          : isWeekend
            ? "weekend"
            : "normal";
        ...
        const weeklyRow = await db("attendance_records")
          ...
          .first()
          .catch(() => null);
        const alreadyAccruedWeeklyMinutes = Number(
          (weeklyRow as any)?.total || 0,
        );
        ...
        if (overtimeMinutes > 0) {
          const autoApproveEnabled = Boolean(
            (otRule as any).autoOtApprove ?? (otRule as any).auto_ot_approve,
          );
          const minMins =
            (otRule as any).autoApproveMinMinutes ??
            (otRule as any).auto_approve_min_minutes;
          const maxMins =
            (otRule as any).autoApproveMaxMinutes ??
            (otRule as any).auto_approve_max_minutes;
      ...
      logger.warn(
        "[AttendanceService.checkOut] OT calculation failed (non-fatal):",
        otErr,
      );
    }
    // ── END AUTO OT ────────────────────────────────────────────────────────
    ...
    record = await this.recordRepo.update(ctx, record.id, {
      ...
      ...(checkoutEvaluation?.status
        ? { status: checkoutEvaluation.status }
        : {}),
      is_early_departure: Boolean(
        checkoutEvaluation &&
        checkoutEvaluation.fullMinutes > 0 &&
        workDurationMinutes < checkoutEvaluation.fullMinutes,
      ),
    });
  ...
  async breakIn(
    ...
    const record = await this.recordRepo.getByEmployeeAndDate(
      ctx,
      input.employeeId,
      today,
    );
    ...
    const activeBreak = existingBreaks.find(
      (b) => b.status === "active" || b.status === "paused",
    );
    if (activeBreak) {
    ...
    try {
      ...
      if (
        assignedShift &&
        (assignedShift.break_duration_minutes ||
          assignedShift.breakDurationMinutes)
      ) {
    ...
    if (totalUsed >= assignedBreakMinutes) {
      throw new ValidationError(
        `Daily break quota for today (${assignedBreakMinutes} Mins) has already been fully used.`,
      );
    }
  ...
  async resumeBreak(ctx: TenantContext, employeeId: number): Promise<any> {
    ...
    try {
      ...
      console.warn(
        "[AttendanceService] Failed to log break_resume session:",
        e,
      );
    }
  ...
  async breakOut(
    ...
    const rawStartTime =
      activeBreak.break_start_time ||
      (activeBreak as any).breakStartTime ||
      activeBreak.created_at;
    let breakStartTime = NaN;
    ...
    const resolvedBreakType =
      options?.breakTypeName || activeBreak.break_type || "General Break";

    await this.breakRepo.update(ctx, activeBreak.id, {
      break_end_time: now,
      break_duration_minutes: breakDurationMinutes,
      break_type: resolvedBreakType,
      ...(options?.breakSettingId
        ? { break_setting_id: options.breakSettingId }
        : {}),
      status: "completed",
    } as any);
  ...
  private async attachLocationNames(ctx: TenantContext, records: any[]) {
    ...
    try {
      ...
      const [locationsAtt, geofences, locationsGen, branches] =
        await Promise.all([
          db("attendance_locations")
            .where("organization_id", ctx.organizationId)
            .whereNull("deleted_at")
            .catch(() => []),
          db("attendance_geofences")
            .where("organization_id", ctx.organizationId)
            .catch(() => []),
          db("locations")
            .where("organization_id", ctx.organizationId)
      ...
      return records.map((rec: any) => {
        ...
        const checkInLocName = inLocId
          ? locMap.get(Number(inLocId)) ||
            rec.check_in_location_name ||
            rec.checkInLocationName ||
            `Location ${inLocId}`
          : rec.check_in_location_name || rec.checkInLocationName || null;
        const checkOutLocName = outLocId
          ? locMap.get(Number(outLocId)) ||
            rec.check_out_location_name ||
            rec.checkOutLocationName ||
            `Location ${outLocId}`
          : rec.check_out_location_name || rec.checkOutLocationName || null;
  ...
  async getHistory(
    ...
    const res = await this.recordRepo.getEmployeeHistory(
      ctx,
      employeeId,
      options,
    );
  ...
  async getByDateRange(
    ...
    const res = await this.recordRepo.getByDateRange(
      ctx,
      employeeId,
      startDate,
      endDate,
  ...
  async getCheckInStatus(ctx: TenantContext, employeeId: number) {
    ...
    try {
      resolvedShift = await this.shiftService.getEmployeeShift(
        ctx,
        employeeId,
        today,
      );
      if (
        resolvedShift &&
        (resolvedShift.break_duration_minutes ||
          resolvedShift.breakDurationMinutes)
      ) {
        assignedBreakMinutes = Number(
          resolvedShift.break_duration_minutes ||
            resolvedShift.breakDurationMinutes,
    ...
    if (record) {
      try {
        ...
        totalBreakMinutes = await this.breakRepo.getTotalBreakDuration(
          ctx,
          record.id,
        );
      } catch (e) {
    ...
    const remainingBreakMinutes = Math.max(
      0,
      assignedBreakMinutes - totalBreakMinutes,
    );
    ...
    const liveEntry = resolvedShift
      ? evaluateShiftEntry(resolvedShift, today, nowForStatus, false)
      : computeShiftEntryStatus({}, nowForStatus, today);
    ...
    const shiftInfo = resolvedShift
      ? {
          shiftName:
            resolvedShift.shift_name || resolvedShift.shiftName || null,
          startTime:
            resolvedShift.start_time || resolvedShift.startTime || null,
          endTime: resolvedShift.end_time || resolvedShift.endTime || null,
          gracePeriodMinutes: Number(
            resolvedShift.grace_period_minutes ??
              resolvedShift.gracePeriodMinutes ??
              0,
          ),
          durationHours: Number(
            resolvedShift.duration_hours ?? resolvedShift.durationHours ?? 8.5,
          ),
    ...
    return {
      ...
      isCheckedOut:
        !!record && !!(record.checkOutTime ?? record.check_out_time),
      checkInTime: record ? (record.checkInTime ?? record.check_in_time) : null,
      checkOutTime: record
        ? (record.checkOutTime ?? record.check_out_time)
        : null,
      duration: record
        ? (record.durationMinutes ?? record.duration_minutes)
        : null,
  ...
  async getReportFilterOptions(
    ...
    try {
      ...
      const companies =
        companyRows.length > 0
          ? companyRows.map((c: any) => ({
              id: String(c.companyId ?? c.company_id),
              name: c.name,
            }))
          : [
              {
                id: String(ctx.organizationId),
                name: currentOrg?.name || "Primary Organization",
              },
            ];
      ...
      const locationRows = await locationQuery
        .select("id", "name")
        .orderBy("name", "asc")
        .catch(() => []);
      ...
      const departmentRows = await departmentQuery
        .select("id", "name")
        .orderBy("name", "asc")
        .catch(() => []);
      ...
      const filterDepts =
        inputDeptIds && inputDeptIds.length > 0 ? inputDeptIds : departmentIds;
      if (filterDepts && filterDepts.length > 0) {
        employeeQuery = employeeQuery.whereIn(
      ...
      console.error(
        "[AttendanceService] Error in getReportFilterOptions:",
        error,
      );
      return {
        companies: [
          { id: String(ctx.organizationId), name: "Primary Organization" },
        ],
        locations: [],
        departments: [],
        reportingOfficers: [],
  ...
  async getTabularReportData(ctx: TenantContext, params: any) {
    ...
    const sf =
      typeof statusFilters === "string"
        ? (() => {
            try {
              return JSON.parse(statusFilters);
            } catch {
              return {};
            }
          })()
    ...
    const rawRo =
      params?.reportingOfficers ??
      params?.["reportingOfficers[]"] ??
      params?.reportingOfficerId ??
      params?.reporting_officer_id;
    const rawCompany =
      params?.companies ??
      params?.["companies[]"] ??
      params?.companyId ??
    ...
    const isAllCompanies =
      String(rawCompany).includes("all") ||
      rawCompany === "all" ||
      !rawCompany ||
    ...
    if (
      ...
      if (filterStatus === "active") {
        empQuery = empQuery.whereIn("status", [
          "active",
          "onboarding",
          "probation",
          "notice",
        ...
        ["inactive", "terminated", "exit", "alumni"].includes(filterStatus)
      ) {
    ...
    if (targetLocIds.length > 0) {
      empQuery = empQuery.where(function () {
        this.whereIn("current_branch_id", targetLocIds).orWhereIn(
          "current_location_id",
          targetLocIds,
        );
      });
    ...
    const [dbRecords, shiftAssignments, breakRows, approvedLeaveRows] =
      await Promise.all([
        db("attendance_records")
          .leftJoin(
            "attendance_locations as in_loc",
            "attendance_records.check_in_location_id",
            "in_loc.id",
          )
          .leftJoin(
            "attendance_locations as out_loc",
            "attendance_records.check_out_location_id",
            "out_loc.id",
          )
          .leftJoin(
            "employees as emp",
          ...
          .leftJoin(
            "attendance_locations as emp_loc",
            "emp.current_location_id",
            "emp_loc.id",
          )
          .leftJoin(
            "shift_templates as st_rec",
        ...
        db("leave_applications")
          ...
          .whereNull("deleted_at")
          .where(function () {
            this.where("application_start_date", "<=", endStr).andWhere(
    ...
    for (const br of breakRows) {
      ...
      if (br.status === "completed" && br.break_duration_minutes != null) {
        ...
        const endMs = pauseMoment
          ? new Date(pauseMoment).getTime()
          : Date.now();
    ...
    for (const leave of approvedLeaveRows) {
      ...
      while (cursor <= end) {
        ...
        if (key >= startStr && key <= endStr)
          leaveMap.set(`${employeeId}_${key}`, leave);
    ...
    const defaultOrgShift =
      shiftTemplatesList.find((s: any) => s.is_default || s.isDefault) ||
      shiftTemplatesList[0];

    const locationNameMap = new Map<number, string>();
    locationsGen.forEach((l: any) =>
      locationNameMap.set(Number(l.id), l.name || l.location_name),
    );
    locationsAtt.forEach((l: any) =>
      locationNameMap.set(Number(l.id), l.location_name || l.name),
    );
    ...
    geofencesList.forEach((g: any) => {
      ...
      const gName =
        g.geofence_name || g.geofenceName || g.location_name || g.locationName;
    ...
    try {
      ...
      console.warn(
        "[TabularReport] Failed to fetch holiday set (non-fatal):",
        e,
      );
    }
    ...
    for (const dateStr of dates) {
      ...
      for (const emp of employeeList) {
        ...
        const normalizeLocationName = (
          ...
          if (
            !locName ||
            locName === "Primary Office" ||
            locName === "Primary Office - Corporate HQ"
          ) {
        ...
        const empRawLoc =
          emp.location_name ||
          emp.locationName ||
          dbRec?.emp_location_name ||
          dbRec?.empLocationName;
        ...
        if (dbRec) {
          ...
          const outLocId =
            dbRec.check_out_location_id || dbRec.checkOutLocationId;

          const rawInLoc =
            (inLocId ? locationNameMap.get(Number(inLocId)) : null) ||
            dbRec.check_in_location_name ||
            dbRec.checkInLocationName ||
            dbRec.check_in_location ||
            dbRec.checkInLocation ||
            dbRec.location;
          const rawOutLoc =
            (outLocId ? locationNameMap.get(Number(outLocId)) : null) ||
            dbRec.check_out_location_name ||
            dbRec.checkOutLocationName ||
            dbRec.check_out_location ||
            dbRec.checkOutLocation ||
            dbRec.location;
          ...
          let durationMins = 0;
          if (
            dbRec.work_duration_minutes !== undefined &&
            dbRec.work_duration_minutes !== null
          ) {
            durationMins = Number(dbRec.work_duration_minutes);
          } else if (
            dbRec.duration_minutes !== undefined &&
            dbRec.duration_minutes !== null
          ) {
            ...
            if (
              ...
              durationMins = Math.floor(
                (dOut.getTime() - dIn.getTime()) / (1000 * 60),
              );
            }
          }
          ...
          const recStoredBreakMins = Number(
            dbRec.break_time_minutes || dbRec.breakTimeMinutes || 0,
          );
          const recBreakMins =
            mapBreakMins > 0 ? mapBreakMins : recStoredBreakMins;
        ...
        if (sf) {
          ...
          if (hasPositiveStatusFilter) {
            ...
            if (isTrue(sf.present) && dayStatus === "Full Day")
              statusMatch = true;
            if (isTrue(sf.halfDay) && dayStatus === "Half Day")
              statusMatch = true;
            if (isTrue(sf.absent) && dayStatus === "Absent") statusMatch = true;
            if (isTrue(sf.leave) && dayStatus === "Leave") statusMatch = true;
            if (
              isTrue(sf.expected) &&
              (dayStatus === "Week Off" || dayStatus === "Holiday")
            )
              statusMatch = true;
        ...
        const departmentName = deptId
          ? deptMap.get(Number(deptId)) || "General"
          : "General";
        ...
        const resolveEmpShift = (
          ...
          if (
            recordObj &&
            (recordObj.rec_shift_name ||
              recordObj.shift_name ||
              recordObj.shiftName)
          ) {
            const sName =
              recordObj.rec_shift_name ||
              ...
              recordObj.shiftName;
            const sIn =
              recordObj.rec_shift_start_time || recordObj.shift_start_time
                ? formatDisplayTime(
                    recordObj.rec_shift_start_time ||
                      recordObj.shift_start_time,
                  )
                : "--";
            const sOut =
              recordObj.rec_shift_end_time || recordObj.shift_end_time
                ? formatDisplayTime(
                    recordObj.rec_shift_end_time || recordObj.shift_end_time,
                  )
                : "--";
            const dur = Number(
              recordObj.rec_shift_duration_hours ||
                recordObj.recShiftDurationHours ||
                recordObj.duration_hours ||
                0,
            );
            return {
              shiftName: sName,
          ...
          const empAssignments = shiftAssignments.filter(
            (sa: any) => Number(sa.employee_id || sa.employeeId) === eId,
          );

          if (empAssignments.length > 0) {
            // Find assignment covering the date
            const dateMatch = empAssignments.find((sa: any) => {
              const sDate = sa.assignment_start_date
                ? getDateStrKey(sa.assignment_start_date)
                : "";
              const eDate = sa.assignment_end_date
                ? getDateStrKey(sa.assignment_end_date)
                : "";
            ...
            if (saMatch) {
              const sName =
                saMatch.shift_name || saMatch.shiftName || "Standard Shift";
              const sIn =
                saMatch.start_time || saMatch.startTime
                  ? formatDisplayTime(saMatch.start_time || saMatch.startTime)
                  : "--";
              const sOut =
                saMatch.end_time || saMatch.endTime
                  ? formatDisplayTime(saMatch.end_time || saMatch.endTime)
                  : "--";
              const dur = Number(
                saMatch.duration_hours || saMatch.durationHours || 9,
              );
              return {
                shiftName: sName,
                startTime: sIn,
          ...
          if (defaultOrgShift) {
            const sName =
              defaultOrgShift.shift_name ||
              defaultOrgShift.name ||
              "General Shift";
            const sIn = defaultOrgShift.start_time
              ? formatDisplayTime(defaultOrgShift.start_time)
              : "09:00 AM";
            const sOut = defaultOrgShift.end_time
              ? formatDisplayTime(defaultOrgShift.end_time)
              : "06:00 PM";
            const dur = Number(
              defaultOrgShift.duration_hours ||
                defaultOrgShift.durationHours ||
                9,
            );
            return {
              shiftName: sName,
        ...
        const expectedMinutes = Math.round(
          (currentShift.durationHours || 0) * 60,
        );
        const [actualHoursPart = "0", actualMinutesPart = "0"] =
          actualWorkingHours.split(":");
        const actualMinutes =
          Number(actualHoursPart) * 60 + Number(actualMinutesPart);
  ...
  async getTimelogMatrixReportData(ctx: TenantContext, params: any) {
    const { fromDate, toDate, status: filterStatus } = params || {};
    ...
    const saturdayRule =
      params?.saturdayRule || params?.saturday_rule || "all_off";
    ...
    if (filterStatus && filterStatus !== "choose" && filterStatus !== "both") {
      if (
        ["active", "inactive", "onboarding", "terminated"].includes(
          filterStatus,
        )
      ) {
        empQuery = empQuery.where("status", filterStatus);
    ...
    if (targetLocIds.length > 0) {
      empQuery = empQuery.where((builder) => {
        builder
          .whereIn("current_branch_id", targetLocIds)
    ...
    locationsList.forEach((l: any) =>
      branchMap.set(l.id, l.name || l.location_name),
    );
    ...
    for (const l of approvedLeaveRows) {
      ...
      const sStr = String(
        l.application_start_date || l.applicationStartDate || "",
      ).slice(0, 10);
      const eStr = String(
        l.application_end_date || l.applicationEndDate || "",
      ).slice(0, 10);
      if (eId && sStr && eStr) {
    ...
    try {
      ...
      console.warn(
        "[MatrixReport] Failed to fetch holiday set (non-fatal):",
        e,
      );
    }
    ...
    return employeeList.map((emp: any) => {
      ...
      const empCode =
        emp.employee_code ||
        emp.employeeCode ||
        `EMP${String(empId).padStart(4, "0")}`;
      const empBranchId =
        emp.current_branch_id ||
        emp.currentBranchId ||
        emp.current_location_id ||
        emp.currentLocationId;
      const location =
        (empBranchId ? branchMap.get(Number(empBranchId)) : null) ||
        defaultLocName;
      ...
      dates.forEach((dateStr, dIdx) => {
        ...
        if (!weeklyWorkMins[weekNum]) {
          weeklyWorkMins[weekNum] = 0;
          weeklyWorkDays[weekNum] = 0;
        }
        ...
        if (dayOfWeek === 6) {
          ...
          if (saturdayRule === "all_working" || saturdayRule === "working") {
            ...
            saturdayRule === "alternate_off" ||
            saturdayRule === "second_fourth_off"
          ) {
            isSaturdayOff =
              saturdayOccurrence === 2 || saturdayOccurrence === 4;
          } else if (saturdayRule === "first_third_off") {
            isSaturdayOff =
              saturdayOccurrence === 1 || saturdayOccurrence === 3;
          } else {
            isSaturdayOff = true;
          }
        }

        const isWeekendOffDay =
          dayOfWeek === 0 || (dayOfWeek === 6 && isSaturdayOff);
        ...
        if (!rec) {
          ...
          if (approvedLeave) {
            const lCode = String(
              approvedLeave.leave_type_code ||
                approvedLeave.leave_type_name ||
                "",
            ).toUpperCase();
        ...
        if (wDuration != null && wDuration > 0) {
          ...
          (rec.check_in_time || rec.checkInTime) &&
          (rec.check_out_time || rec.checkOutTime)
        ) {
        ...
        const breakMins =
          Number(rec.break_time_minutes ?? rec.breakTimeMinutes) || 0;
      ...
      const weekNums = [
        ...new Set(dates.map((_, dIdx) => Math.floor(dIdx / 7) + 1)),
      ];
  ...
  async getCeoPunches(ctx: TenantContext): Promise<any[]> {
    ...
    const ceoName = ceoEmp
      ? [
          ceoEmp.firstName || ceoEmp.first_name,
          ceoEmp.lastName || ceoEmp.last_name,
        ]
          .filter(Boolean)
          .join(" ")
      : [
          adminUser?.firstName || adminUser?.first_name,
          adminUser?.lastName || adminUser?.last_name,
        ]
          .filter(Boolean)
          .join(" ") ||
        org?.ownerName ||
        ...
        "Harsh Gawali (CEO)";
    const ceoCode =
      ceoEmp?.employee_code ||
      ceoEmp?.employeeCode ||
      `CEO-${ctx.organizationId}-${adminUserId}`;
    ...
    const dbRecords = await db("attendance_records")
      ...
      .where("attendance_records.organization_id", ctx.organizationId)
      .where(function () {
        this.where("attendance_records.is_ceo_punch", 1).orWhere(
          "attendance_records.is_ceo_punch",
          true,
        );
        if (ceoEmpId) {
    ...
    return dbRecords.map((r: any) => {
      ...
      if (rawIn) {
        ...
        if (!isNaN(dIn.getTime())) {
          formattedIn = dIn.toLocaleTimeString("en-US", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: true,
          });
      ...
      if (rawOut) {
        ...
        if (!isNaN(dOut.getTime())) {
          formattedOut = dOut.toLocaleTimeString("en-US", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: true,
          });
      ...
      return {
        ...
        checkInLocation:
          ...
          r.notes ||
          "Executive Boundary / Headquarters",
