...
console.log('Selected employee (default):', empLabel);
...
const currentCtcText = await page.locator('text=Current CTC:').textContent();
console.log('Current CTC label:', currentCtcText);

await page.locator('input[value="Annual compensation review and performance adjustment"]').fill('QA_Test_Revision_Reason');

await page.screenshot({ path: `${outDir}/rev_03_filled.png`, fullPage: true });

await page.locator('button:has-text("Submit Revision Request")').click();
await page.waitForTimeout(2500);
await page.screenshot({ path: `${outDir}/rev_04_after_submit.png`, fullPage: true });

console.log('NETWORK:', JSON.stringify(netlog, null, 2));

// Reload fresh to verify persistence
await page.reload({ waitUntil: 'domcontentloaded' });
await page.waitForTimeout(2500);
await page.screenshot({ path: `${outDir}/rev_05_after_reload.png`, fullPage: true });
