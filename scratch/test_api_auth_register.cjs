...
async function testApiWithLogin() {
  try {
    ...
    console.log('Sending GET /payroll/process-register?cycleId=5&month=2026-09...');
    const regRes = await axios.get('http://localhost:5000/api/v1/payroll/process-register?cycleId=5&month=2026-09', { headers });
    ...
    if (regRes.data.data?.employees?.length > 0) {
      for (const emp of regRes.data.data.employees) {
        ...
        console.log(`- Dynamic Component Values Calculated:`);
...
testApiWithLogin().catch(console.error);
